  ...
  CheckForUpdates() {
    try {
      ...
      if (VerCompare(NewestVersion, VERSION) == 1) {
        ...
        VersionGui.Add(
          "Text",
          "",
          "Update Available.`nCurrent version: " VERSION "`nNew version available: " NewestVersion "`n`nContinue with update? It will only take a moment, and the script will automatically restart."
        )
    ...
    UpdateScript(GuiCtrl, *) {
      ...
      try {
        ...
        LogError(
          "Failed to download update. Please check your internet connection and try again. Optionally send an issue at: " GithubLink,
          e,
          true
        )
        ExitApp()
      ...
      try {
        ...
        LogError(
          "Failed to extract updated files from .zip. Try again or send an issue at: " GithubLink,
          e,
          true
        )
        ExitApp()
  ...
  try {
    ...
    if (found) {
      if (FileGetSize(tempFile) > 0) {
        ...
        MsgBox(
          "Could not terminate TCP connections with cports. Closing game to be safe. This should not happen. Send an issue in: " GithubLink
        )
      }
    } else {
    ...
    ProcessClose(Game.PID)
    LogError(
      "Something went wrong when trying to close TCP connections. Closing game to be safe. This should not happen. Send an issue in: " GithubLink,
      e
    )
  } finally {
    loop 20 {
